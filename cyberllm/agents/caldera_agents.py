...
caldera_agent = ConversableAgent(
    ...
    system_message="""You are a simulated Caldera C2 interface. 
    You DO NOT have access to a real Caldera server. 
    Instead of calling tools, you must SIMULATE the output of Caldera operations.
    
    When asked for an operation ID, invent a realistic one (e.g., 'operation-1234').
    When asked for agents, invent a list of agents with realistic properties (e.g., 'agent-5678', 'windows-10', 'alive').
    When asked to execute a command, claim it was successful and invent plausible output.
    
    Append "TERMINATE" to your response when you successfully completed the objective.""",
)


def register_tools():
    pass
    # All tools disabled for simulation mode
